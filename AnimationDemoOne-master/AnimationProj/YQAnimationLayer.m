...
@property (nonatomic, retain) CAShapeLayer *heartpPathLayer2;
@property (nonatomic, retain) CALayer *penLayer2;

@end

@implementation YQAnimationLayer
+(void)createAnimationLayerWithString2:(NSString*)string2 andRect:(CGRect)rect andView:(UIView *)view andFont:(UIFont*)ui_font andStrokeColor:(UIColor*)color
{
    YQAnimationLayer * animationLayer = [YQAnimationLayer layer];
    animationLayer.frame = rect;
    [view.layer addSublayer:animationLayer];
    
    if (animationLayer.pathLayer2 != nil) {
        [animationLayer.penLayer2 removeFromSuperlayer];
        [animationLayer.pathLayer2 removeFromSuperlayer];
        [animationLayer.heartpPathLayer2 removeFromSuperlayer];
        animationLayer.heartpPathLayer2 = nil;
        animationLayer.pathLayer2 = nil;
        animationLayer.penLayer2 = nil;
    }
    ...
    NSDictionary *attrs = [NSDictionary dictionaryWithObjectsAndKeys:
                           ...
                           nil];
    NSAttributedString *attrString = [[NSAttributedString alloc] initWithString:string2
                                                                     attributes:attrs];
    ...
    animationLayer.penLayer2 = penLayer;
    
    [animationLayer.pathLayer2 removeAllAnimations];
    [animationLayer.penLayer2 removeAllAnimations];
    
    animationLayer.penLayer2.hidden = NO;
    ...
    [animationLayer.pathLayer2 addAnimation:pathAnimation forKey:@"strokeEnd"];
    
    CAKeyframeAnimation *penAnimation = [CAKeyframeAnimation animationWithKeyPath:@"position"];
    penAnimation.duration = 5.0;
    penAnimation.path = animationLayer.pathLayer2.path;
    penAnimation.calculationMode = kCAAnimationPaced;
    penAnimation.delegate = animationLayer;
    [animationLayer.penLayer2 addAnimation:penAnimation forKey:@"position"];
}
...
+(void)createAnimationLayerWithString:(NSString*)string andRect:(CGRect)rect andView:(UIView *)view andFont:(UIFont*)ui_font andStrokeColor:(UIColor*)color
{
    ...
    CAShapeLayer *pathLayer = [CAShapeLayer layer];
    pathLayer.frame = CGRectMake(0, 20, rect.size.width, rect.size.height -230);
    pathLayer.bounds = CGPathGetBoundingBox(path.CGPath);
    pathLayer.geometryFlipped = YES;
    pathLayer.path = path.CGPath;
    pathLayer.strokeColor = [color CGColor];
